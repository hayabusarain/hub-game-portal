 ...
 * ■ 寸法。**1800px 幅で作ること**
 *
 * カードは高さ 176px の枠に object-cover で上端基準に収める。見えるのは上から
 * 4〜5割だけなので、被写体は上半分に置く。幅は 1800px 必要で、比率は問わない。
 *
 * 高精細な画面ではこの枠が 1536px の実ピクセルになる。900px で作っていると
 * 約1.7倍に引き伸ばされ、目に見えて甘くなる。3枚とも 900px で作られていて、
 * MLBB のカードが荒いという指摘から 2026-09-11 に判明した。
 * honor-of-kings.jpg（900×413）と wild-rift.jpg（900×531）は元素材が手元に
 * 無いため 900px のまま。素材が手に入ったら 1800px で作り直す。
 *
 * mobile-legends.jpg は 1800×826。公式サイト（mobilelegends.com）のヒーロー面が
 * 出している Kalea の painting（750×721、背景は黒）を 1.5 倍に拡大し、地の
 * グラデーションへ screen 合成したもの。絵そのものには手を入れていない。
 * 作り方は scripts/make_game_banner.mjs にあり、拡大率を決めた根拠もそこに書いた。
 *
 * ■ 権利。弱いのは MLBB の1枚
 *
 ...
 *
 * ワイルドリフトの1枚は事情が違う。Riot はファンコンテンツの方針を公開しており、
 * 拠りどころがある（方針の条文そのものは当方で再確認していない）。
 * つまり3枚の中で MLBB がいちばん弱い。削除要請が来たらまずこの1枚を外す。
 *
