...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return { title: t('wildRift.title'), description: t('wildRift.description'), alternates: getAlternates(locale, '/guides/wild-rift') };
}

export default async function WildRiftGuidePage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  setRequestLocale(locale);
  const tCommon = await getTranslations('GuidesPage');
  const t = await getTranslations('GuideWildRift');
    ...
    <div className="min-h-screen bg-slate-50 text-slate-700 flex flex-col font-sans">
      ...
      <main className="flex-1 w-full max-w-3xl mx-auto px-5 py-10 space-y-8">
        ...
        <article className="space-y-6 text-slate-700 leading-relaxed text-sm md:text-base font-normal">
          ...
          <GlossaryTermLinks termKeys={['Ward', 'FogOfWar', 'LastHit', 'Objective', 'Kiting', 'Dive', 'Macro', 'Scaling']} />
        </article>
      </main>
